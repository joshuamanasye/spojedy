...
<template>
    <!-- Only renders when a song has been loaded -->
    <div v-if="currentSong" class="fixed bottom-0 left-0 right-0 z-40">

        <!-- ── Main bar ── -->
        <div class="bg-[#F9F8F4] dark:bg-[#141310] border-t border-[#E2DDD4] dark:border-[#2E2B25]">

            <!-- Progress bar — full width, sits at the very top of the bar -->
            <div
            ...
            <!-- Controls row -->
            <div class="max-w-5xl mx-auto px-5 pb-4 flex items-center gap-4">

                <!-- Left: album art + song info -->
                <button
                    @click="router.push(`/song/${currentSong.id}`)"
                    class="flex items-center gap-3 w-56 shrink-0 cursor-pointer text-left group/info"
                ...
                >
                    <img
                        ...
                        :class="[
                            'w-11 h-11 object-cover shrink-0 border border-[#E2DDD4] dark:border-[#2E2B25]',
                ...
                <!-- Center: prev / play / next + time -->
                <div class="flex-1 flex items-center justify-center gap-2">
                    <button
                        @click="prevSong"
                        class="w-10 h-10 flex items-center justify-center cursor-pointer text-[#8A8679] dark:text-[#7A7870] hover:text-[#1A1916] dark:hover:text-[#EDE9DF] transition-colors"
                        title="Previous"
                    >
                    ...
                    <button
                        @click="togglePlay"
                        class="w-10 h-10 flex items-center justify-center cursor-pointer bg-[#1A1916] dark:bg-[#EDE9DF] text-[#F9F8F4] dark:text-[#141310] hover:bg-[var(--accent)] dark:hover:bg-[var(--accent)] dark:hover:text-white transition-colors"
                        title="Play / Pause"
                    >
        ...
        <!-- ── Queue panel — fixed right overlay, stops above the player bar ── -->
        <Transition name="queue-slide">
            <div
                v-if="showQueue"
                class="fixed top-14 right-0 bottom-20 w-72 z-50 flex flex-col bg-[#F9F8F4] dark:bg-[#141310] border-l border-[#E2DDD4] dark:border-[#2E2B25] shadow-[-12px_0_32px_rgba(0,0,0,0.06)]"
            >
                <!-- Panel header -->
                ...
                <div class="flex-1 overflow-y-auto">
                    ...
                    <ul v-else>
                        ...
                        >
                            ...
                            <img :src="song.cover" :alt="song.title" class="w-8 h-8 object-cover shrink-0" />
